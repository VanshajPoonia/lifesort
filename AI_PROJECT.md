...
- AI "What Am I Ignoring?" Insights at `/insights` and `/api/ai/what-am-i-ignoring`: authenticated GET returns non-AI neglect/risk signals for quiet Life Areas, stale goals/projects, overdue waiting items/commitments/maintenance, missed habits, upcoming Vault renewals, and finance review gaps. Authenticated POST calls OpenRouter for read-only explanations and small suggested actions. Rate-limited to 5/day. Suggested actions become tasks only after explicit user confirmation.
