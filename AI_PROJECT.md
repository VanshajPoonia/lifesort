...
- Generic tags (`tags`, `item_tags` tables; `/api/tags`, `/api/item-tags`; `components/tag-picker.tsx`) for object types that have no tagging otherwise — currently `task`/`goal`/`project`. Deliberately additive and separate from the existing `TEXT[]` tag columns on `notes`/`people`/`vault_items`/`budget_transactions`, which are untouched (see `AI_DECISIONS.md`). UI wired into `app/tasks/page.tsx`, the Goal detail modal (`components/goal-modal.tsx`), and the Project detail page (`app/projects/[id]/page.tsx`) — deliberately **not** wired into Notes, which already has its own freeform tag field (2026-07-24, see `AI_TASK_LOG.md`).
- Generic attachments (`attachments` table; `/api/attachments`, `/api/attachments/[id]`; `lib/r2.ts`; `components/attachment-list.tsx`) for `task`/`goal`/`project`/`note`/`vault_item`. Files are stored in a private Cloudflare R2 bucket (not Postgres) and accessed only via short-lived server-issued presigned URLs after a per-item ownership check — the bucket has no public domain and no file bytes ever pass through our own API. UI wired into `app/vault/page.tsx`, the Goal detail modal, the Project detail page, and `app/notes/page.tsx` (2026-07-24). **`R2_*` env vars are only in local `.env.local`, not yet added to Vercel** — this feature is not deploy-ready until that's done and the exposed credential is rotated (see `AI_TASK_LOG.md` 2026-07-23 19:40 IST entry).
...
- Tasks with quick-create Enter submission, priority, a 7-value workflow **status** (Inbox/Next/In Progress/Waiting/Someday/Completed/Cancelled, its own filter dropdown), due date/time **and** a separate scheduled date/time + estimated duration, reminders, completion state (kept in sync with status), category, optional goal linking, optional Life Area assignment, quick date filters, multi-select bulk actions, project linking through `project_items`, persisted manual drag ordering, Tags (`TagPicker`/`item_tags`), a collapsible subtasks/checklist panel (`task_checklist_items`, own drag-reorder), a **recurrence rule** (`task_recurrence`: daily/weekdays/weekly/monthly/yearly/custom interval, fixed-schedule or repeat-from-completion, optional end date/count) editable from the same dialog as due/scheduled/duration, a collapsible **Depends on** panel (task-to-task dependencies via `item_relationships`' `depends_on` relation, not a separate table), and a collapsible **Related items** panel (`components/related-items.tsx`, cross-type links to Goals/Projects/Notes/Domains via `item_relationships`' `related` relation — see below). Completing a recurring task reopens it at the next occurrence instead of leaving it completed. `AI_BUILD_PLAN.md` Phase 1 "Tasks depth" is fully done (2026-07-25, see `AI_TASK_LOG.md`/`AI_DECISIONS.md`). Scheduled date/duration/recurrence/dependencies are Tasks-page-only; `/today` and `/calendar` still schedule by due date.
- **Related items** — a generic "Related items" panel (`components/related-items.tsx`) on Task cards, the Goal edit modal, the Project detail page, the Notes editor, and the Domain detail page, backed by the `item_relationships` table's `related` relation (cross-type links among these same 5 object types; display labels resolved server-side by `GET /api/item-relationships`). Distinct from Tasks' own **Depends on** panel above (`depends_on` relation, task-to-task only, ordered waiting-on/blocks semantics). `AI_BUILD_PLAN.md` Phase 1 ("Core organization") is fully done as of this addition (2026-07-26, see `AI_TASK_LOG.md`/`AI_DECISIONS.md`).
