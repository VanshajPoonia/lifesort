...
- `npm test` runs Vitest (`vitest run`). Config: `vitest.config.ts` (Node environment, `@/` alias, dummy `DATABASE_URL` test env so `lib/auth.ts`/`lib/db.ts` import cleanly without a real database). Suites as of 2026-07-25: `lib/auth.test.ts` (12), `lib/date-math.test.ts` (11), `app/api/tags/route.test.ts` (8), `app/api/item-relationships/route.test.ts` (10), `app/api/tasks/[id]/checklist-items/route.test.ts` (17), `app/api/tasks/[id]/recurrence/route.test.ts` (12), `app/api/tasks/route.test.ts` (32) — **102 tests total**. `app/api/tasks/route.ts` imports `neon` from `@/lib/neon-client` (see below), same as every other DB-touching file now — its test mocks `@/lib/neon-client` directly rather than `@/lib/db`, since it still doesn't use the shared `sql` export, unlike every other route test in this repo.
