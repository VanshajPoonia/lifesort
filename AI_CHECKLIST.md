...
- `npm run lint` runs via `eslint.config.mjs` (flat config, imports `eslint-config-next/core-web-vitals` + `eslint-config-next/typescript` directly — no `FlatCompat` needed since `eslint-config-next@16` ships native flat arrays). Requires `eslint@9.x` — **ESLint 10 is not yet supported by `eslint-plugin-react`** (confirmed via its published `peerDependencies`), so do not bump past ESLint 9 until that changes. Started at 293 pre-existing findings (first-ever lint run against this codebase); as of 2026-07-24 14:50 IST, `react/no-unescaped-entities` and `@typescript-eslint/no-unused-vars` are fully cleared, bringing it to 185 problems (148 errors, 37 warnings). As of 2026-07-24 evening, 186 problems (`task-checklist.tsx` added one instance of the already-deferred `react-hooks/set-state-in-effect` category). As of 2026-07-25, **187 problems, unchanged since** — `components/task-dependencies.tsx` added one more instance of that same already-deferred category (see `AI_DECISIONS.md`); the DATE-timezone fix (below) touched 50+ files' import lines and added zero new findings. Remaining categories (`react-hooks/set-state-in-effect`, `react-hooks/immutability`, `react-hooks/exhaustive-deps`, `@typescript-eslint/no-explicit-any`, `react-hooks/purity`, `@next/next/no-img-element`) need case-by-case behavioral review, not mechanical fixes — see `AI_TASK_LOG.md` for the breakdown.
...
- **Known pre-existing bug, severity confirmed 2026-07-25 (still not fixed):** `@neondatabase/serverless` serializes Postgres `DATE` columns (`due_date`, `scheduled_date`, and likely others) as JS `Date` objects built in the server process's **local** timezone, so `.toISOString()`/`JSON.stringify()` shifts the date back by a day whenever that process's `TZ` isn't UTC (confirmed on local dev, which runs IST). This is **not just a display issue** — `dateOnly()` in `app/api/tasks/route.ts` re-derives the shifted value from any row read and it gets written back on every save, and it can make `daily`/interval-1 task recurrence net zero calendar advance per cycle (confirmed against the live DB — see `AI_DECISIONS.md`'s "Tasks Depth Sub-step 3" entry). Almost certainly invisible in production (Vercel functions run UTC), but affects any non-UTC local dev environment more seriously than previously assessed. See `AI_DECISIONS.md` for the full root-cause writeup, why it wasn't fixed as part of Tasks depth (systemic across every DATE column, not scoped to one feature), and a suggested fix approach for a future dedicated task.
