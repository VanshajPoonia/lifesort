...
- `npm test` runs Vitest (`vitest run`). Config: `vitest.config.ts` (Node environment, `@/` alias, dummy `DATABASE_URL` test env so `lib/auth.ts`/`lib/db.ts` import cleanly without a real database). Suites as of 2026-07-24: `lib/auth.test.ts` (12), `app/api/tags/route.test.ts` (8), `app/api/item-relationships/route.test.ts` (10), `app/api/tasks/[id]/checklist-items/route.test.ts` (17), `app/api/tasks/route.test.ts` (23) — **70 tests total**. `app/api/tasks/route.ts` imports `neon` directly (not `@/lib/db`'s shared `sql`) — its test mocks `@neondatabase/serverless`'s `neon` export directly rather than `@/lib/db`, unlike every other route test in this repo.
- `npm run lint` runs via `eslint.config.mjs` (flat config, imports `eslint-config-next/core-web-vitals` + `eslint-config-next/typescript` directly — no `FlatCompat` needed since `eslint-config-next@16` ships native flat arrays). Requires `eslint@9.x` — **ESLint 10 is not yet supported by `eslint-plugin-react`** (confirmed via its published `peerDependencies`), so do not bump past ESLint 9 until that changes. Started at 293 pre-existing findings (first-ever lint run against this codebase); as of 2026-07-24 14:50 IST, `react/no-unescaped-entities` and `@typescript-eslint/no-unused-vars` are fully cleared, bringing it to 185 problems (148 errors, 37 warnings). As of 2026-07-24 evening, **186 problems** — `components/task-checklist.tsx` added one consistent instance of the already-deferred `react-hooks/set-state-in-effect` category (see `AI_DECISIONS.md`); the later Tasks depth sub-step 2 changes added zero new findings (still 186). Remaining categories (`react-hooks/set-state-in-effect`, `react-hooks/immutability`, `react-hooks/exhaustive-deps`, `@typescript-eslint/no-explicit-any`, `react-hooks/purity`, `@next/next/no-img-element`) need case-by-case behavioral review, not mechanical fixes — see `AI_TASK_LOG.md` for the breakdown.
- **Known pre-existing bug (not yet fixed):** `@neondatabase/serverless` serializes Postgres `DATE` columns (`due_date`, `scheduled_date`, and likely others) as JS `Date` objects built in the server process's **local** timezone, so `.toISOString()`/`JSON.stringify()` shifts the date back by a day whenever that process's `TZ` isn't UTC (confirmed on local dev, which runs IST). Almost certainly invisible in production (Vercel functions run UTC), but affects any non-UTC local dev environment. See `AI_DECISIONS.md`'s "Tasks Depth Sub-step 2" entry for the full root-cause writeup and why it wasn't fixed as part of that task (it's systemic across every DATE column, not scoped to one feature).
...
- `npm run lint` now runs (see "Test/lint status" above) and exits non-zero due to 186 pre-existing findings (down from 293 as of 2026-07-24 14:50 IST), not a config failure.
- `npm test` now runs Vitest and passes (70/70 tests).
