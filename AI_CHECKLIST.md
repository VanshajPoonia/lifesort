...
Test/lint status (updated 2026-07-25 — see `AI_TASK_LOG.md` for the full setup story):

- `npm test` runs Vitest (`vitest run`). Config: `vitest.config.ts` (Node environment, `@/` alias, dummy `DATABASE_URL` test env so `lib/auth.ts`/`lib/db.ts` import cleanly without a real database). Suites as of 2026-07-26: `lib/auth.test.ts` (12), `lib/date-math.test.ts` (11), `app/api/tags/route.test.ts` (8), `app/api/item-relationships/route.test.ts` (12), `app/api/tasks/[id]/checklist-items/route.test.ts` (17), `app/api/tasks/[id]/recurrence/route.test.ts` (12), `app/api/tasks/route.test.ts` (32) — **104 tests total**. `app/api/tasks/route.ts` imports `neon` from `@/lib/neon-client` (see below), same as every other DB-touching file now — its test mocks `@/lib/neon-client` directly rather than `@/lib/db`, since it still doesn't use the shared `sql` export, unlike every other route test in this repo. `app/api/item-relationships/route.test.ts`'s hoisted `sql` mock also carries a `.unsafe()` stub (`(value) => value`) — needed because `lib/item-relationships.ts`'s `attachRelationshipLabels()` calls real `sql.unsafe()` for dynamic table/column names and isn't itself mocked away, unlike `validateItemOwnership`.
- `npm run lint` runs via `eslint.config.mjs` (flat config, imports `eslint-config-next/core-web-vitals` + `eslint-config-next/typescript` directly — no `FlatCompat` needed since `eslint-config-next@16` ships native flat arrays). Requires `eslint@9.x` — **ESLint 10 is not yet supported by `eslint-plugin-react`** (confirmed via its published `peerDependencies`), so do not bump past ESLint 9 until that changes. Started at 293 pre-existing findings (first-ever lint run against this codebase); as of 2026-07-24 14:50 IST, `react/no-unescaped-entities` and `@typescript-eslint/no-unused-vars` are fully cleared, bringing it to 185 problems (148 errors, 37 warnings). As of 2026-07-24 evening, 186 problems (`task-checklist.tsx` added one instance of the already-deferred `react-hooks/set-state-in-effect` category). As of 2026-07-25 daytime, 187 problems (`components/task-dependencies.tsx` added one more instance of that same category; the DATE-timezone fix touched 50+ files' import lines and added zero new findings). As of 2026-07-25 evening, **164 problems (127 errors, 37 warnings)** — all 23 `@typescript-eslint/no-explicit-any` findings fixed (see `AI_DECISIONS.md`'s "Production-Level Hardening Pass" entry). Remaining categories (`react-hooks/set-state-in-effect`, `react-hooks/immutability`, `react-hooks/exhaustive-deps`, `react-hooks/purity`, `@next/next/no-img-element`) still need case-by-case behavioral review, not mechanical fixes — see that same entry for why (mostly React-Compiler-readiness rules flagging currently-correct code, not runtime bugs; `no-img-element` is low-value here given `images.unoptimized: true`).
- **`DATE` columns and `@neondatabase/serverless` — always import `neon` from `@/lib/neon-client`, never directly from `"@neondatabase/serverless"`.** As of 2026-07-25 (see `AI_DECISIONS.md`'s "DATE-Timezone Bug: Root-Cause Fix" entry), `lib/neon-client.ts` registers a type parser so Postgres `DATE` columns arrive as plain `"YYYY-MM-DD"` strings instead of JS `Date` objects (which previously round-tripped through local-server-midnight construction and shifted by a day on any non-UTC server, on every read *and* every subsequent write). A `Next.js` `instrumentation.ts` hook was tried first and does **not** work for this — confirmed Next.js bundles each API route separately from the instrumentation entry point, so a registration there doesn't reach a route's own queries. `lib/date-math.ts` has the shared UTC-explicit day/month arithmetic (`addDaysToDate`, `addMonthsToDate` with month-end clamping, `daysBetweenDates`) that any date-recurrence-style feature should reuse rather than re-deriving with `new Date(str + "T00:00:00")` + `.toISOString()`, which reintroduces the same bug.
- **`next.config.mjs`: `typescript.ignoreBuildErrors` is now `false`** (flipped 2026-07-25 evening — `tsc --noEmit` is clean, so `npm run build` now actually fails on real type errors instead of silently shipping them). `eslint.ignoreDuringBuilds` is still `true` — see the lint count above. Raw-SQL-row placeholder types (`type Row = Record<string, any>`) should be written as `Record<string, unknown>` going forward, with a precise `as <type>` cast at the point a field is actually consumed (e.g. `row.field as string | number | Date` before `new Date(...)`) — not a blanket cast, and not `any`. Exception: a file that renders one generic row shape across many different entity types directly as JSX (see `app/domains/[id]/page.tsx`) needs a concrete scalar union (`Record<string, string | number | boolean | null | undefined>`) instead, since `unknown` can't satisfy `ReactNode` without narrowing at every render call site.
- **The DATE-timezone bug below is now fixed** (as of 2026-07-25 evening) — see `AI_DECISIONS.md`'s "DATE-Timezone Bug: Root-Cause Fix" entry above this note. The original bug description is kept below for historical context on what was wrong and why it mattered, but treat it as resolved, not open.
- Historical bug description (now fixed, kept for context): `@neondatabase/serverless` serialized Postgres `DATE` columns (`due_date`, `scheduled_date`, and others) as JS `Date` objects built in the server process's **local** timezone, so `.toISOString()`/`JSON.stringify()` shifted the date back by a day whenever that process's `TZ` wasn't UTC (confirmed on local dev, which runs IST). This was **not just a display issue** — `dateOnly()`-style helpers re-derived the shifted value from any row read and it got written back on every save, and it could make `daily`/interval-1 task recurrence net zero calendar advance per cycle (confirmed against the live DB). Almost certainly invisible in production (Vercel functions run UTC). See `AI_DECISIONS.md` for the full root-cause writeup and the fix.
- **Manual browser verification without a project Playwright dependency:** this repo has no `playwright`/`puppeteer` devDependency and no `chromium-cli` on PATH, but Playwright's CLI/browsers are available via npx's own cache. To drive the real app in a headless browser for UI verification (see the Tasks Depth Sub-step 3 entry for a full example — register a disposable account, don't touch the real user's session): write a script and run it with `npx --yes --package=playwright node <script.js>` (plain `node -e "require('playwright')"` inline eval does NOT resolve — it must be a file). Browsers were already cached locally at `~/Library/Caches/ms-playwright/`; if they aren't on a fresh machine, `npx playwright install chromium` downloads them.
...
- `npm run lint` now runs (see "Test/lint status" above) and exits non-zero due to 187 pre-existing findings (down from 293 as of 2026-07-24 14:50 IST), not a config failure.
- `npm test` now runs Vitest and passes (102/102 tests).
