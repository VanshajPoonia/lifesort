  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        {[
          { href: "/money?tab=income",      label: "Income Sources",  desc: `${incomeSources.length} source${incomeSources.length !== 1 ? "s" : ""}`,   icon: <TrendingUp className="h-4 w-4 text-success" /> },
          { href: "/money?tab=investments", label: "Investments",     desc: `${investments.length} position${investments.length !== 1 ? "s" : ""}`,      icon: <BarChart3 className="h-4 w-4 text-success" /> },
          { href: "/money?tab=wishlist",    label: "Wishlist",        desc: `${wishlistItems.filter(i=>!i.purchased).length} item${wishlistItems.filter(i=>!i.purchased).length !== 1 ? "s" : ""} pending`, icon: <Heart className="h-4 w-4 text-pink-500" /> },
