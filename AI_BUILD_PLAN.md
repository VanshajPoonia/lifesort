...
- ✅ **Relationships UI** — generic "Related items" panel (`components/related-items.tsx`, relation type `related`) surfaced on Task/Goal/Project/Note/Domain via `item_relationships` (2026-07-26, see `AI_TASK_LOG.md` and `AI_DECISIONS.md`). Cross-type linking among these same 5 types; display labels resolved server-side by the API. Distinct from the existing `TaskDependencies` component (relation `depends_on`, task-to-task only) — both now render on task cards.
- **Exit:** a task can carry due≠scheduled≠duration, subtasks, a recurrence rule, and related items, all persisted; Tags/Attachments usable beyond their first surface. **Met — Phase 1 is fully ✅ as of 2026-07-26.**
