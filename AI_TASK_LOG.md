...
Current verification state (2026-07-26):

- `npm run build` passes with TypeScript validation now actually enforced (`next.config.mjs`'s `typescript.ignoreBuildErrors` is `false`; `tsc --noEmit` is clean). `eslint.ignoreDuringBuilds` remains `true` — 66 pre-existing lint findings remain (down from 293 originally; see the 2026-07-25 ESLint cleanup entry), every one a reviewed-and-commented `react-hooks/set-state-in-effect` exception (either a real state transition or a function intentionally shared with non-effect call sites). It still exits non-zero; that is expected and not a blocker.
...
- `npm test` runs Vitest: 104/104 tests passing across 7 suites (`lib/auth.test.ts`, `lib/date-math.test.ts`, `app/api/tags/route.test.ts`, `app/api/item-relationships/route.test.ts`, `app/api/tasks/[id]/checklist-items/route.test.ts`, `app/api/tasks/[id]/recurrence/route.test.ts`, `app/api/tasks/route.test.ts`).
- `npx tsc --noEmit` is clean.

## Completed Work

### 2026-07-26 - Relationships UI — Related Items Panel (AI_BUILD_PLAN.md Phase 1 complete)

- **Agent/tool used:** Claude Code (Sonnet 5), primary coding agent (`CLAUDE.md` fallback mode, per the user's standing override), per the user's explicit "yeah let's finish phase 1" after a status-check identified the Relationships UI bullet as the one item blocking Phase 1 from being fully ✅.
- **Task completed:** built a generic "Related items" panel backed by the existing `item_relationships` table (added Phase 0, previously only consumed by `task-dependencies.tsx`'s `depends_on` relation) and surfaced it on all 5 object types the Phase 1 exit criteria names: Task, Goal, Project, Note, Domain. `AI_BUILD_PLAN.md` Phase 1 ("Core organization") is now fully ✅.
- **Research before implementing:** read `app/api/item-relationships/route.ts`, `lib/item-relationships.ts`, and `components/task-dependencies.tsx` (the existing precedent) directly. Spawned an Explore agent in parallel to map the exact title/name column for `tasks`/`goals`/`projects`/`notes`/`life_areas` in `scripts/schema.sql`, confirm the existing `GET /api/goals`/`/api/projects`/`/api/notes`/`/api/life-areas` list endpoints could power an "add related item" picker as-is (they can — all four return a plain array of `{id, title|name, ...}` for the current user, no new endpoints needed), and locate the exact insertion point + id-variable type on each of the 4 non-Task pages.
- **Design decisions (recorded in full in `AI_DECISIONS.md`'s new "Relationships UI" entry):**
  - Scoped to `relation: 'related'` only (not the other 5 relation types the schema allows) and to the 5 named types (not all 17) — avoids duplicating `TaskDependencies`' `depends_on` UI and avoids guessing column names for types nothing writes yet.
  - Display labels resolved **server-side** (`lib/item-relationships.ts`'s new `attachRelationshipLabels()`, wired into `GET /api/item-relationships`) rather than client-side, since the panel can mount once per task card — server-side resolution is bounded by relationship count per item, not by the size of every candidate list.
  - N+1 small per-row label queries (parallelized via `Promise.all`), not a batched `ANY(...)` query — reuses the exact single-id query shape already proven in `validateItemOwnership`, on the reasoning that per-item relationship counts are small in a personal-productivity panel.
  - The "add related item" picker's candidate lists are fetched lazily per type (only when that type is picked) and cached, reusing the existing `/api/tasks`/`/api/goals`/`/api/projects`/`/api/notes`/`/api/life-areas` list endpoints rather than adding new ones.
- **Files changed:**
  - `lib/item-relationships.ts` — added `LABEL_COLUMN_BY_TYPE` (5 verified types only) and `attachRelationshipLabels()`.
  - `app/api/item-relationships/route.ts` — `GET` now runs relationship rows through `attachRelationshipLabels()` before returning, adding a `label: string | null` field per row.
  - `app/api/item-relationships/route.test.ts` — extended the hoisted `sql` mock with a `.unsafe()` method (the first test in this file to exercise real, non-mocked-away `sql.unsafe`-calling code) and added 2 new tests: label resolution across multiple rows/directions, and the null-label fallback when the linked item no longer exists.
  - `components/related-items.tsx` (new) — the generic panel, structurally mirroring `task-dependencies.tsx` (collapsed-by-default toggle, `let cancelled`-guarded fetch-on-mount effect, `useToast` on failure) but with a two-step type-then-item "add" picker instead of one.
  - `app/tasks/page.tsx`, `components/goal-modal.tsx`, `app/projects/[id]/page.tsx`, `app/notes/page.tsx`, `app/domains/[id]/page.tsx` — one import + one render call each, placed next to the existing `AttachmentList`/`TaskDependencies` call on each page.
  - `AI_BUILD_PLAN.md`, `AI_DECISIONS.md`, `AI_PROJECT.md`, `AI_TASK_LOG.md` (this entry) — documentation.
- **Commands run:**
  - `npx tsc --noEmit` — 2 real type errors caught and fixed: (1) the test file's `.unsafe()`-augmented mock lost its `vi.fn()` mock-method types (`mockReset`/`mockResolvedValueOnce`/etc.) after an `as unknown as {...}` cast — fixed by typing it as `ReturnType<typeof vi.fn> & { unsafe: ... }` instead of erasing to a bare function signature; (2) `app/projects/[id]/page.tsx`'s `projectId` (from `useParams()`) is `string | undefined`, not assignable to `RelatedItems`' `itemId: string | number` — fixed by using the already-narrowed `project.id` instead (same pattern the neighboring `AttachmentList` call on the same page already uses). Clean after both fixes.
  - `npm run build` — clean, all routes compile.
  - `npm test` — **104/104 passed** (2 new, up from 102).
  - `npx eslint .` — **66 problems, unchanged** from the prior session's baseline; `related-items.tsx` itself has zero findings.
  - Live verification against the real dev server with a disposable account (registered via `/register`, deleted via `psql DELETE FROM users ... RETURNING id` afterward, cascade-verified with a follow-up orphan-row check — see below): seeded one of each of the 5 linkable types, linked task→note and project→domain via direct API calls, confirmed labels resolve correctly **and bidirectionally** (both sides of a relationship show the correct resolved label and correct `direction`), confirmed DELETE removes it from both sides, then drove the real UI (Playwright, `npx --package=playwright`) through all 5 surfaces: Domain page and Project page confirmed via UI click-through against pre-seeded links; Task page's full "add via picker" flow (expand → pick type → pick item → Add) confirmed end-to-end including the resulting POST and the label appearing in the list; Goal modal and Notes page confirmed rendering the panel and (for Goal) showing a linked item correctly.
- **Bugs found or fixed (all self-caught during verification, none shipped):**
  - `sql.unsafe is not a function` in the new test cases — the route test's hoisted `sql` mock was a bare `vi.fn()` with no `.unsafe` method, and `attachRelationshipLabels()` (unlike the already-mocked `validateItemOwnership`) actually runs its real implementation in tests. Fixed by adding `.unsafe` to the mock (see Commands above).
  - **A test-methodology mistake, caught and corrected before being trusted:** the first live-UI pass of the Tasks-page "add" picker asserted success by waiting for the picked item's label text to appear on screen — but a `<Select>`'s own trigger displays the just-picked option's label immediately on selection, before "Add" is even clicked, so the assertion passed regardless of whether the POST actually succeeded. Direct API/DB checks afterward showed the relationship had **not** been created on that first pass. A slower, explicitly-instrumented repeat (logging the actual `POST /api/item-relationships` network call and its response) succeeded and the row was confirmed via `psql`. Root cause was rapid synthetic clicks outrunning Radix `Select`'s open/close portal animation in headless automation, not a defect in `related-items.tsx` — recorded in `AI_DECISIONS.md` as a general lesson: a UI text-match is not proof of a write succeeding, confirm against the actual API/DB response for write-path verification.
  - **A real, minor leak, fixed immediately:** while extracting `DATABASE_URL` for the disposable-account cleanup, an initial `cat .env.local | cut -d= ...` approach preserved the literal surrounding quote characters from the `.env.local` file, producing an invalid connection string whose error message echoed the full credential (including password) into the tool transcript. Fixed by switching to `source .env.local` (proper shell quote-parsing) for all subsequent DB commands. Flagged to the user directly in-session. No credential was written to any file or committed; the exposure was confined to this local conversation's tool output. Given the account is a local Neon dev database, the user may still want to consider rotating it — their call.
- **Remaining issues or known limitations:** the panel only supports the `related` relation among 5 types (by design, see `AI_DECISIONS.md`); `backlink`/`mention`/`source_of`/`converted_from` remain unused, reserved for later Phase 2/3 automation. No cycle/duplicate prevention beyond excluding already-linked items in the picker (matches `TaskDependencies`' existing precedent). `AI_BUILD_PLAN.md` Phase 1 is now fully ✅; Phase 2 ("Daily planning") is the next open phase.
- **Suggested next steps:** per `AI_BUILD_PLAN.md`, Phase 2 is next: universal Inbox processing actions + NL parsing, Today-as-command-center depth (scratchpad→object conversion, drag-resize, end-of-day reflection), Upcoming view, Suggested planning, and recurrence materialization. No specific sub-step has been chosen yet — needs the user's go-ahead on scope/sequencing, same as Phase 1's sub-steps were.
- **Handoff notes:** `attachRelationshipLabels()` and `LABEL_COLUMN_BY_TYPE` in `lib/item-relationships.ts` are the pattern to extend if a 6th linkable type is ever added to `RelatedItems` — add the column mapping there and to `related-items.tsx`'s `LINKABLE_TYPES` array; both are deliberately partial/allowlist-shaped (not derived from the full 17-type `itemRelationshipTypes`) so an unmapped type fails safe (null label) rather than guessing. The disposable-account DB cleanup convention should always go through `source .env.local` (or equivalent proper shell-quote parsing), never `cat`/`cut` on the raw file — see the leak note above.
...
- **Agent/tool used:** Claude Code (Sonnet 5), primary coding agent (`CLAUDE.md` fallback mode), per the user's explicit instruction to fix bugs found along the way before moving to the next feature, after choosing "Full audit, then global fix" when asked how to scope it.
- **Task completed:** Root-caused and fixed the DATE-timezone bug documented (but deliberately not fixed) in the two prior entries below. DATE columns no longer round-trip through JS `Date` objects at all — they arrive everywhere as plain `"YYYY-MM-DD"` strings, eliminating the local/UTC ambiguity entirely instead of working around it per-callsite.
- **Investigation before implementing:** confirmed the exact mechanism with a standalone script (`SELECT '2026-08-01'::date` came back as a `Date` whose `.toISOString()` was `"2026-07-31T18:30:00.000Z"` on this IST machine). Discovered `@neondatabase/serverless` re-exports `pg-types`' `types.setTypeParser`, and verified via another standalone script that registering a parser for DATE (OID 1082) that returns the raw string works, including for `neon()` clients created *before* the registration call. Then discovered the real complication: **60+ files each create their own `neon()` client independently** (`grep`-counted) — `lib/db.ts`'s shared client is the minority pattern, not the norm. A global fix has to reach every one of them.
- **Full blast-radius audit before shipping** (done directly via grep + targeted reads, not a subagent, per explicit user preference this session): read every DATE-typed column out of `scripts/schema.sql`, then checked every server-side call site (`app/api/**/route.ts`, `lib/**/*.ts`) that calls a Date-instance method (`.toISOString()`, `.getMonth()`, `.getDate()`, `.getFullYear()`, `.getDay()`, `.setDate()`, `.setMonth()`) on a DB-sourced value, to make sure flipping DATE columns from `Date` objects to strings wouldn't break anything expecting a `Date` instance. Found **zero risky call sites** — every consumer already guards with an `instanceof Date` ternary (the `dateOnly()`/`formatDateOnly()`/`toIsoDate()` pattern, already present consistently across ~12 files), which falls through correctly once `instanceof Date` becomes false. Found two *separate* pre-existing instances of the same underlying local-midnight-then-UTC-read anti-pattern (unrelated to the fix's safety, since neither reads a DB `Date` object at all) and fixed those too — see below.
- **First attempt didn't work, and that's a real finding:** tried a Next.js `instrumentation.ts` `register()` hook first (the textbook "run once at server boot" mechanism). It ran (confirmed via a log line) but had zero effect on `app/api/tasks/route.ts`'s actual queries — direct debug logging showed `existing.due_date` was still a shifted `Date` object. Registering the identical call *inside* `app/api/tasks/route.ts` fixed it immediately. Conclusion: Next.js bundles the `instrumentation` entry point separately from each API route, so they can end up running against different in-memory copies of `@neondatabase/serverless`'s module state. Deleted `instrumentation.ts` rather than leave a fix that silently does nothing.
- **Files changed:**
  - `lib/neon-client.ts` (new) — wraps `neon` from `@neondatabase/serverless`, registers `types.setTypeParser(types.builtins.DATE, (value) => value)` as a module-load side effect, re-exports `neon`. This is now the only file allowed to import `@neondatabase/serverless` directly.
  - **49 route files + `lib/db.ts` + `lib/auth.ts` + `lib/ai-usage.ts`** — changed `import { neon } from "@neondatabase/serverless"` to `import { neon } from "@/lib/neon-client"` (mechanical, done via a scripted `sed` pass across the whole list, then verified with `tsc`/tests/build — not hand-edited file by file).
  - `app/api/tasks/route.ts` — extracted `addDaysToDate`/`addMonthsToDate`/`daysBetweenDates` out into the new `lib/date-math.ts` (see below) instead of keeping its own copy now that a second file needs the identical functions; behavior unchanged, confirmed by its existing 32 tests passing without modification.
  - `lib/date-math.ts` (new) — the extracted, shared UTC-explicit date-math helpers, with `lib/date-math.test.ts` (new, 11 tests).
  - `app/api/maintenance/complete/route.ts` — `nextDueDate()` rewritten to use the shared helpers instead of its own local-midnight-then-`.toISOString()` arithmetic (same bug pattern, found during the audit, not caused by the parser fix).
  - `app/api/journal/weekly-digest/route.ts` — `weekBounds()` rewritten the same way.
  - `app/api/tasks/route.test.ts` — mock target changed from `@neondatabase/serverless` to `@/lib/neon-client` to match the route's new import; same 32 tests otherwise unmodified.
  - `AI_DECISIONS.md`, `AI_TASK_LOG.md` (this entry) — documentation. `AI_CHECKLIST.md`/`AI_PROJECT.md` updated where the matrix applies.
- **Commands run:**
  - `npx tsc --noEmit` — clean, no output, run twice (after the import migration, and again after the two bonus bug fixes).
  - `npm run build` — clean, all ~140 routes including the new/changed ones, no errors or warnings.
  - `npm test` — **102/102 passed** (11 new for `lib/date-math.test.ts`, up from 91).
  - Manual re-verification: re-ran the exact Playwright + `psql` recurrence walkthrough from the sub-step 3 entry below against a fresh disposable account. This time `tasks.due_date` genuinely advanced from `2026-08-01` to `2026-08-02` on completing a daily-recurring task (previously stuck at `2026-08-01` — see that entry's finding). Also spot-checked `people.birthday` through a different route to confirm a second DATE column round-trips correctly. Deleted all disposable accounts created during this session's testing (9 total across both sub-step 3 and this fix) afterward.
- **Bugs found:**
  - The DATE-timezone bug itself — fixed, see above.
  - Confirmed (not caused): `app/api/maintenance/complete/route.ts` and `app/api/journal/weekly-digest/route.ts` had the same local-midnight-then-UTC-read anti-pattern independently. Fixed alongside the main fix since they're the identical bug class, found during the same audit.
  - **New, unrelated bug found by chance, not fixed:** `POST /api/people` 500s (`NeonDbError: malformed array literal`) whenever `tags` is a non-empty array — the driver serializes it as `["qa"]` instead of Postgres's `{qa}` array-literal syntax. This is an array-column serialization issue, unrelated to DATE/timezone, discovered only because a birthday spot-check happened to exercise this route. Added to Active Bugs / Issues below; needs its own scoped task.
- **Remaining issues / known limitations:** the `/api/people` tags-array bug above is unfixed. No test coverage added for `app/api/maintenance/complete/route.ts` or `app/api/journal/weekly-digest/route.ts` specifically (neither had any before; the shared date-math they now call is tested directly instead — see `AI_DECISIONS.md` for the reasoning on why a full new test suite for two previously-untested routes was treated as separate scope). The DATE-timezone fix is unverified against a real Vercel/production deploy (only verified locally, on IST) — should be a non-event there per the existing "Vercel likely runs UTC" assumption, but that assumption itself remains unverified.
- **Suggested next steps:** fix the newly-found `/api/people` tags-array bug as its own small scoped task. Otherwise, return to the Phase 1 "Relationships UI" bullet (the one remaining open item before Phase 1 is fully ✅) or whatever the user directs next — this entry is a pure bug fix with no product-scope change of its own.
- **Handoff notes:** any *new* file that needs its own Postgres connection must import `neon` from `@/lib/neon-client`, never directly from `@neondatabase/serverless` — a direct import silently reintroduces the DATE-timezone bug for that file with no error or warning. If a future change ever needs to register another custom type parser, do it inside `lib/neon-client.ts` (or another module every route actually imports), never inside `instrumentation.ts` — see the "doesn't work" finding above before assuming otherwise.
...
## 2026-07-25 - ESLint Cleanup Pass (react-hooks/set-state-in-effect, immutability, purity, exhaustive-deps)

- Agent/tool used: Claude Code (Sonnet 5), continuing a task started in an earlier session (same day) that got the count from 164 to ~107 across `daily-popup.tsx`, `wishlist-panel.tsx`, `links/page.tsx`, `today/page.tsx`, `tasks/page.tsx`, `projects/page.tsx`, `investments-panel.tsx`, `budget-panel.tsx`, `share/[token]/page.tsx`, `whiteboard/page.tsx`, and `nuke/page.tsx`. This session picked up from that handoff, verified `nuke/page.tsx` (was unconfirmed), and worked through the remaining ~57 files.
- Task completed: Reduced the 293 pre-existing `npm run lint` findings noted in `AGENTS.md` (2026-07-24) down to 66, all in the `react-hooks/set-state-in-effect` rule (part of the React Compiler ESLint rules bundled with `eslint-config-next@16.2.11`). Every `react-hooks/immutability` (function-used-before-declared), `react-hooks/purity` (`Date.now`/`Math.random` during render), `react-hooks/use-memo`, and `react-hooks/exhaustive-deps` finding in the app is now resolved.
- Files changed (66): `app/admin/page.tsx`, `app/ai-chat/page.tsx`, `app/calendar/page.tsx`, `app/commitments/page.tsx`, `app/custom-sections/page.tsx`, `app/daily-content/page.tsx`, `app/domains/[id]/page.tsx`, `app/domains/page.tsx`, `app/habits/page.tsx`, `app/inbox/page.tsx`, `app/insights/page.tsx`, `app/journal/page.tsx`, `app/links/page.tsx`, `app/maintenance/page.tsx`, `app/money/page.tsx`, `app/notes/page.tsx`, `app/notifications/page.tsx`, `app/nuke/page.tsx`, `app/page.tsx`, `app/people/page.tsx`, `app/pomodoro/page.tsx`, `app/projects/[id]/page.tsx`, `app/projects/page.tsx`, `app/reset/page.tsx`, `app/review/page.tsx`, `app/rules/page.tsx`, `app/settings/page.tsx`, `app/share/[token]/page.tsx`, `app/someday/page.tsx`, `app/spaces/[id]/page.tsx`, `app/spaces/page.tsx`, `app/tasks/page.tsx`, `app/templates/page.tsx`, `app/timeline/page.tsx`, `app/today/page.tsx`, `app/vault/page.tsx`, `app/waiting/page.tsx`, `app/whiteboard/[id]/page.tsx`, `app/whiteboard/page.tsx`, `app/whiteboard/share/[token]/page.tsx`, `app/workspace/page.tsx`, `components/attachment-list.tsx`, `components/auth-provider.tsx`, `components/daily-popup.tsx`, `components/dashboard-layout.tsx`, `components/domain-focus-provider.tsx`, `components/games/snake-game.tsx`, `components/games/wordle-game.tsx`, `components/global-command-palette.tsx`, `components/global-search.tsx`, `components/goal-modal.tsx`, `components/hub-page.tsx`, `components/money/budget-panel.tsx`, `components/money/income-panel.tsx`, `components/money/investments-panel.tsx`, `components/money/wishlist-panel.tsx`, `components/quick-add-modal.tsx`, `components/subscription-checker.tsx`, `components/task-checklist.tsx`, `components/task-dependencies.tsx`, `components/theme-switcher.tsx`, `components/ui/carousel.tsx`, `components/ui/sidebar.tsx`, `components/ui/use-mobile.tsx`, `components/whiteboard/share-dialog.tsx`, `hooks/use-mobile.ts`.
- Summary of changes and why, by finding type:
  - `react-hooks/immutability` ("X is accessed before it is declared"): the flagged fetch/load function was defined below the `useEffect` that called it. Fixed by moving the function above the effect and wrapping it in `useCallback` with the correct dependency array, then adding it to the effect's deps.
  - `react-hooks/purity` (`Date.now()`/`Math.random()` called during render): in `pomodoro.tsx` and `notes/page.tsx`, the call was inside a plain (non-memoized) function or a `useMemo` — resolved by wrapping the containing function in `useCallback`, or (for `notes/page.tsx`'s recency filter and `sidebar.tsx`'s decorative skeleton width) replacing the inline call with a `useState` value computed once and, where the value needs to stay fresh, refreshed on an interval.
  - `react-hooks/use-memo` (`useMemo(namedFn, deps)` instead of an inline arrow): fixed in `habits/page.tsx` by wrapping the call — `useMemo(() => namedFn(), deps)`.
  - `react-hooks/exhaustive-deps`: fixed by adding the now-`useCallback`-wrapped function to the relevant dependency array; a few required also memoizing a second function it referenced (e.g. `pomodoro.tsx`'s `durations`/`handleSessionTypeChange`) to avoid the fix itself triggering a new "changes every render" warning.
  - `react-hooks/set-state-in-effect` (66 remaining, all consciously kept): this rule flags two shapes — a literal `setState(...)` call in an effect body, and (less obviously) *any* effect-body call to a function that calls `setState` anywhere in its body, even after an `await`. For each occurrence the fix depended on which shape and whether the state transition was real:
    - **Redundant transition, single caller** (state already starts at the value being set, function/effect only meaningfully runs once): deleted the redundant call. Used sparingly — most cases below were not actually redundant.
    - **Real transition, single caller, cleanly separable from the effect's own logic** (`custom-sections.tsx`'s `fetchSections`/`fetchLifeAreas`, `maintenance.tsx`, `notifications/page.tsx`, `someday/page.tsx`'s `loadLifeAreas`, `templates/page.tsx`'s `loadMyTemplates`, `whiteboard/[id]/page.tsx`, `whiteboard/share/[token]/page.tsx`, `auth-provider.tsx`, `dashboard-layout.tsx`'s `fetchSidebarPrefs`, `task-checklist.tsx`, `task-dependencies.tsx`): rewrote the function to return data instead of calling `setState` internally, and had the effect apply the result behind a `let cancelled = false` guard (same pattern as the prior session's `nuke/page.tsx` fix). This is a real fix, not a suppression — these files have zero lint findings now.
    - **Real transition, but the function is also called from a button/handler/other effect that needs the same reset behavior** (the majority of the 66): left the code as-is and added a one-line comment above the flagged call explaining the reuse, e.g. `fetchDashboard is also shared with a manual refresh handler below`. Confirmed reuse by grepping every other call site before deciding — this caught and fixed one mistake mid-session (`whiteboard/[id]/page.tsx`'s `loadBoard` was assumed multi-use from a stale reading, actually had zero other callers, and got the proper return-data refactor instead of a wrong comment).
    - **Real transition driven by a changing dependency** (filters, search queries, `selectedSection`/`selectedDate`/route params, dialogs that reset on open): left as-is with a comment — the setState is genuinely needed every time the effect re-fires, matching the pattern the user explicitly approved in the prior session for `tasks/page.tsx`'s filter effect.
    - **Hydration from an external, synchronous source** (`localStorage`/`sessionStorage`/URL query params on mount): left as-is with a comment — the value being set isn't statically knowable, so there's nothing to make "pure."
- Commands run and results:
  - `npx tsc --noEmit` — clean throughout (checked after every batch of ~8 files, and once at the end for the full repo).
  - `npm test` (Vitest) — 102/102 passing throughout, no regressions.
  - `npm run build` — succeeds, all routes compile.
  - `npx eslint .` — 164 → 66 problems, all `react-hooks/set-state-in-effect`, all individually commented. Zero warnings remain (the two warnings introduced mid-session by the `pomodoro.tsx` and `income-panel.tsx` refactors were both fixed before moving on, not left behind).
- Bugs found or fixed: none pre-existing; this was a lint-only pass. One mistake was made and caught within this session (see `whiteboard/[id]/page.tsx` note above) before it reached the final diff.
- Remaining issues or known limitations:
  - 66 `react-hooks/set-state-in-effect` findings remain by design (see above) — do not try to "finish" this to zero without re-reading the reasoning per file; several of these are the officially-recommended-least-bad option for patterns React's own docs acknowledge don't have a clean effect-free alternative (e.g. syncing state to a changing prop in a reused modal/dialog instance).
  - The 44 lint findings that were already fixed and committed in earlier sessions before this one (2026-07-24 commit `20f5c41` and prior) are not part of this count; this entry covers only the pass that took 164 → 66.
- Suggested next steps:
  - None required — this task is complete. If someone wants to push further, the only remaining lever is the "shared function" cases: splitting a multi-caller fetch function into an effect-only variant (return-data pattern) plus a separately-named imperative variant for handlers would eliminate more of the 66, but it's a larger refactor per file for a cosmetic lint count, not a correctness fix.
  - Un-migrated: none. This pass touched only client components' `useEffect`/`useCallback` usage; no API routes, schema, or architecture changed.
- Handoff notes:
  - The two established, user-approved exceptions to "fix everything" are: (1) a literal `setState` in an effect that reacts to a real, changing dependency (filter/search/route param) — keep, comment; (2) a function shared between an effect and other call sites (buttons, other handlers) where dropping the internal `setState` would break those other call sites — keep, comment. Do not treat either as something to "clean up further."
  - When applying the return-data refactor pattern, always add the `let cancelled = false` guard and clear it in the effect's cleanup — this repo's established pattern from `nuke/page.tsx`, reused across ~12 files in this pass.
  - Before assuming a flagged function is "shared" (multi-use) and reaching for the comment-only fix, grep for every call site first — `whiteboard/[id]/page.tsx`'s `loadBoard` looked shared at a glance but wasn't, and got the wrong initial treatment before being caught and corrected.
