...
- Signed-in pages use `DashboardLayout` with consolidated hub-level navigation. The primary sidebar intentionally shows only Home, Today, Workspace, Money, Reflect, Settings, and admin-only Admin. Individual feature routes remain deep-linkable and are reached from hub cards, Global Search, Quick Add, mobile More, or direct URLs. Sidebar preferences use `workspace` as the current hub key while `organize` remains a legacy fallback for stored user JSON. Hub badges use `/api/navigation-summary`, a read-only user-scoped aggregate endpoint with missing-schema tolerance and no new tables.
- Home is an attention dashboard, not a full module dashboard. Keep it short: greeting, compact Quick Access, compact LifeScore, compact Today and Money summaries, unified Pending, Journal CTA, Recent Activity, and pinned favorites. Quick Access should prefer 4-6 primary actions, move secondary actions behind a menu, and use localStorage for recently used shortcuts instead of database tracking unless a future task explicitly asks for cross-device personalization. Detailed module widgets belong in their feature routes or hubs.
...
- Today is the primary daily focus surface. Focus items, due Today To-Do items, Should Do, and Could Do should appear as one prioritized daily list with Must/Should/Could filters; capacity, habits, calendar, notes, deadlines, and reflection remain supporting sections.
- Workspace is the primary workspace hub at `/workspace`, with `?tab=plan|capture|visual|systems|follow-ups` tabs. `/organize`, `/plan`, and `/life-admin` are compatibility route files that redirect to matching Workspace tabs. `/capture` remains the full Universal Capture feature page because it contains unique workflow logic.
- Reflect is the primary insight/review surface at `/reflect`. `/insights` remains a compatibility feature route with the same tabbed Reflect workflow so old deep links do not break. Life Balance and Ignored Signals reuse the existing in-page logic; Weekly Review, Timeline, LifeScore, Reset, and Coach remain reachable through focused links.
- Mobile navigation uses a bottom bar for Home, Today, Workspace, Money, and More. More exposes Reflect, Settings, Profile, Support/FAQs, and admin-only Admin while the header still keeps Global Search, Quick Add, notifications, profile/settings, and sign-out reachable.
- Whiteboard lives under Workspace > Visual rather than the top-level sidebar. `/whiteboard` remains deep-linkable and highlights Workspace as its active navigation area.
- Spaces live under Workspace > Visual rather than the top-level sidebar. `/spaces` and `/spaces/[id]` remain deep-linkable and highlight Workspace as their active navigation area.
