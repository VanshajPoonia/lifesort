...
- Decision: Replace OpenRouter with direct Google Gemini API calls for all AI text/structured-output features.
  ...
  - Model ids changed from OpenRouter aliases (e.g. `google/gemini-2.0-flash-exp:free`, `openai/gpt-4o-mini`) to native Gemini model ids (`gemini-2.5-flash`, `gemini-2.5-pro`). `lib/ai-models.ts` `AVAILABLE_MODELS` now lists only the two Gemini models; the GPT/Claude/free-router options in the LifeSort Coach model picker no longer exist.
  ...
  - Verify current Gemini model ids against Google's live model list before relying on this decision long-term — model names on the Gemini API change over time and `gemini-2.5-flash`/`gemini-2.5-pro` reflect what was available as of this change.
