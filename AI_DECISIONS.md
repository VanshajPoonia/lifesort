...
- Signed-in pages use `DashboardLayout` with consolidated hub-level navigation. The primary sidebar intentionally shows only Home, Today, Organize, Money, Reflect, Settings, and admin-only Admin. Individual feature routes remain deep-linkable and are reached from hub cards, Global Search, Quick Add, mobile More, or direct URLs. Sidebar preferences control the current hub keys while legacy per-feature preference keys remain harmless for stored user JSON. Hub badges use `/api/navigation-summary`, a read-only user-scoped aggregate endpoint with missing-schema tolerance and no new tables.
- Home is an attention dashboard, not a full module dashboard. Keep it short: greeting/today snapshot, compact LifeScore, compact Today and Money summaries, unified Pending, Recent Activity, and pinned favorites. Detailed module widgets belong in their feature routes or hubs.
- Home supports an account-scoped compact/detailed toggle through `users.app_preferences.home_view_mode` via `/api/app-preferences`. Keep `/api/app-preferences` allowlisted; do not turn it into arbitrary client-writable user JSON.
...
- Whiteboard is a top-level desktop/tablet nav item between Organize and Money because it is a core creative workspace. Mobile keeps the five-slot bottom bar unchanged and exposes Whiteboard through More.
